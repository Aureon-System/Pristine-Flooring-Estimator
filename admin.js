...
function render(){
  ...
  $('#mrr').textContent=money(m.mrr);$('#arr').textContent=money(m.arr);$('#revenue').textContent=money(m.realized_revenue);
  $('#monthlyCosts').textContent=money(m.monthly_costs);$('#margin').textContent=money(m.projected_operating_margin);$('#aiCost').textContent=money(m.ai_cost_month);
  ...
  $('#costRows').innerHTML=(summary.costs||[]).map(c=>'<tr><td>'+esc(c.cost_date)+'</td><td>'+esc(c.category)+'</td><td>'+esc(c.vendor||'—')+'</td><td>'+esc(c.description)+'</td><td>'+esc(c.cadence)+'</td><td>'+money(c.amount)+'</td></tr>').join('')||'<tr><td colspan="6">No costs entered yet.</td></tr>';
