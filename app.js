...
let areas=[],addons=[],brandLogoDraft=null;
...
function renderAreas(){const box=$('#areas');box.innerHTML='';areas.forEach((a,i)=>{if(a.dailyRate===undefined)a.dailyRate=0;if(a.crewSize===undefined)a.crewSize=2;if(a.durationDays===undefined)a.durationDays=1;if(a.surcharge===undefined)a.surcharge=patternSurcharges[a.pattern]??0;const el=document.createElement('div');el.className='area-card';const matOpts=materials.map(m=>`<option ${m===a.material?'selected':''}>${esc(m)}</option>`).join(''),patternOpts=patterns.map(p=>`<option ${p===a.pattern?'selected':''}>${esc(p)}</option>`).join(''),t=areaTotals(a),disabled=a.material==='Material not included'?'disabled':'';el.innerHTML=`<div class="area-head"><div><span class="area-index">AREA ${String(i+1).padStart(2,'0')}</span><strong>${esc(a.description||'Installation area')}</strong></div>${areas.length>1?'<button class="remove-btn" type="button" aria-label="Remove area">×</button>':''}</div><div class="area-grid"><label class="wide"><span class="field-title">Area description</span><input data-k="description" value="${esc(a.description)}"></label><label><span class="field-title">Area (sqft)</span><input data-k="sqft" type="number" min="0" step="1" value="${a.sqft}"></label><label class="wide"><span class="field-title">Material</span><select data-k="material">${matOpts}</select></label><label><span class="field-title">Installation pattern</span><select data-k="pattern">${patternOpts}</select></label></div><div class="crew-grid"><label><span class="field-title">Daily rate / person ($)<small class="internal-tag">INTERNAL</small></span><input data-k="dailyRate" type="number" min="0" step="0.01" value="${a.dailyRate}"></label><label><span class="field-title">Crew size (people)<small class="internal-tag">INTERNAL</small></span><input data-k="crewSize" type="number" min="0" step="1" value="${a.crewSize}"></label><label><span class="field-title">Duration (work days)<small class="internal-spacer">INTERNAL</small></span><input data-k="durationDays" type="number" min="0" step="1" value="${a.durationDays}"></label><label><span class="field-title">Total crew cost<small class="internal-tag">INTERNAL</small></span><input data-derived="crewTotal" value="${money(t.crewTotal)}" disabled></label><label><span class="field-title">Crew cost / sqft<small class="internal-tag">INTERNAL</small></span><input data-derived="crewRate" value="${money(t.crewRate)} / sqft" disabled></label></div><div class="crew-formula">Crew cost / sqft = daily rate per person × crew size × work days ÷ area sqft.</div><div class="pricing-grid"><label><span class="field-title">Selling labor / sqft<small class="internal-spacer">INTERNAL</small></span><input data-k="sellRate" type="number" min="0" step="0.01" value="${a.sellRate}"></label><label><span class="field-title">Pattern surcharge %<small class="internal-spacer">INTERNAL</small></span><input data-k="surcharge" type="number" min="0" step="0.1" value="${a.surcharge}"></label><label><span class="field-title">Effective labor rate<small class="internal-spacer">INTERNAL</small></span><input data-derived="effective" value="${money(t.effective)} / sqft" disabled></label><label><span class="field-title">Material cost / sqft<small class="internal-tag">INTERNAL</small></span><input data-k="materialCost" type="number" min="0" step="0.01" value="${a.materialCost}" ${disabled}></label><label><span class="field-title">Material sell / sqft<small class="internal-spacer">INTERNAL</small></span><input data-k="materialSell" type="number" min="0" step="0.01" value="${a.materialSell}" ${disabled}></label></div><div class="area-total-line"><span data-derived="meta">${Math.round(num(a.sqft)).toLocaleString()} sqft · ${esc(a.pattern)} · ${num(a.durationDays)} work day${num(a.durationDays)===1?'':'s'}${a.material==='Material not included'?' · Material not included':''}</span><b data-derived="total">${money(t.totalSell)}</b></div>`;el.querySelectorAll('[data-k]').forEach(inp=>{const ev=inp.tagName==='SELECT'?'change':'input';inp.addEventListener(ev,()=>{let v=inp.value;if(['sqft','dailyRate','crewSize','durationDays','sellRate','surcharge','materialCost','materialSell'].includes(inp.dataset.k))v=num(v);a[inp.dataset.k]=v;if(inp.dataset.k==='pattern'){a.surcharge=patternSurcharges[v]??0;const s=el.querySelector('[data-k="surcharge"]');if(s)s.value=a.surcharge}if(inp.dataset.k==='material'){renderAreas();return}if(inp.dataset.k==='description')el.querySelector('.area-head strong').textContent=v||'Installation area';syncAreaCard(el,a)})});const rm=el.querySelector('.remove-btn');if(rm)rm.onclick=()=>{areas=areas.filter(x=>x.id!==a.id);renderAreas();calc()};box.append(el)});calc()}
...
function convertSavedToInvoice(d){
  ...
  loadSavedDocument(d,false);
  $('#documentType').value='INVOICE';
  $('#documentNo').value='INV-'+Math.random().toString(36).slice(2,9).toUpperCase();
  $('#issueDate').value=today();
  $('#validThrough').value='';
  calc();
  const invoice={...snapshot(),sourceEstimateNo:d.documentNo,savedAt:new Date().toISOString()};
  const docs=loadDocs();
  docs.unshift(invoice);
  saveDocs(docs);
  document.querySelector('#top')?.scrollIntoView({behavior:'smooth',block:'start'});
  alert('Invoice created from '+d.documentNo+' and saved. You can edit, email or download it now.');
}
